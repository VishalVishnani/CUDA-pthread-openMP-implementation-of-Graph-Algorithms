...
int main(){
    ...
    Host_Graph=(int *)malloc(Graph_Size);
    Host_Path=(int *)malloc(Graph_Size);
    Host_Graph1=(int *)malloc(Graph_Size);
    Host_Path1=(int *)malloc(Graph_Size);
    Edges_Per_Vertex = (int*)malloc(VERTICES*sizeof(int));
    ...
    /*Get the start time for pthread computation*/
    gettimeofday(&initial,NULL);

    for (j = 0; j < VERTICES/NTHREADS; j++) {
        rc=pthread_create(&threads[j],&attr,(void*)Parallel_Floyd,(void*) j);
    ...
    /*Get the stop time for pthread computation*/
    gettimeofday(&final,NULL);
    ...
    /*Check if serial and pthread implementation match*/
    int match=0;
    for(i=0;i<VERTICES*VERTICES;i++){
...
void* Parallel_Floyd(void* j){
    ...
    for(x=0;x<VERTICES;++x){
        for(y=0;y<VERTICES;++y){
            for(z=m;z<m+NTHREADS;++z){    
      		    current_node=y*VERTICES+z;
		        Node_i=y*VERTICES+x;
		        Node_j=x*VERTICES+z;
                if(Host_Graph1[current_node]>(Host_Graph1[Node_i]+Host_Graph1[Node_j])){
                	Host_Graph1[current_node]=(Host_Graph1[Node_i]+Host_Graph1[Node_j]);
                    Host_Path1[current_node]=x;
                }
             }
        }
